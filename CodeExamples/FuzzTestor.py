...
class Fuzz_Testor():
    ...
    def __init_mission_file(self) -> None:
        file_path = os.path.join(os.path.dirname(__file__), "missions", "FUZZ_MISSION2.json")
        f = open(file_path, 'r')
    ...
    def run_test(self,fuzz_test:Fuzz_Test,ones_columns):
        ...
        print('fuzz test space',fuzz_test)
        print()
    ...
    def calculate_rtl_threshold(self):
        ...
        if current_pose is None:
            print("[calculate_rtl_threshold] Position data unavailable; using default (75 sec).")
            return 75
        ...
        # Calculate horizontal distance in the XY plane.
    ...
    def calculate_land_threshold(self):
        ...
        current_pose = self.ros_interface.get_current_pose(timeout=.3)
        if current_pose is None:
            print("[calculate_land_threshold] Current position data unavailable; using default (75 sec).")
            return 75
    ...
    def timer_adjustment_handler(self):
        ...
        while not (self.force_shutdown.is_set() or self.test_complete.is_set()):
            self.adjust_timer_event.wait()  # Block until event is set.
